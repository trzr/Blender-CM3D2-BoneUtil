...
class MESH_OP_select_symmetry(bpy.types.Operator):  # type: ignore
	...
	def execute(self, context):  # type: (bpy.types.Context) -> Set
		...
		def negative_x(co):  # type: ignore
			co.x = -co.x

		def negative_y(co):  # type: ignore
			co.y = -co.y

		def negative_z(co):  # type: ignore
			co.z = -co.z
...
def register():  # type: () -> None
	bpy.types.Scene.TrzrSelUtilOpr = bpy.props.EnumProperty(
	...
	bpy.types.Scene.TrzrSelUtilAxis = bpy.props.EnumProperty(
		name='Axis',
		items=[
			('x', 'x', "", '', 0),
			('y', 'y', "", '', 1),
