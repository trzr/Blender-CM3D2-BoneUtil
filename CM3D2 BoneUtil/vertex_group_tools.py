...
def menu_func_specials(self, context):
	if not common.prefs().vgfeature: return
	
	ob = context.active_object
	if ob is None or ob.type != 'MESH': return
	if ob.parent and ob.parent.type == 'ARMATURE':
		if len(ob.parent.data.bones) == 0 and not 'BaseBone' in ob.parent.data: return
	else:
		if not 'BaseBone' in ob: return
	
	self.layout.separator()
	self.layout.operator(Bone2VertexGroup.bl_idname, icon='BONE_DATA')

class Bone2VertexGroup(bpy.types.Operator):
	...
	src_data = bpy.props.EnumProperty(name="Source",
			items=[
	...
	def execute(self, context):
		...
		for bone_name in target_bonenames:
			if '_IK_' in bone_name: continue
			if bone_name == bb_name: continue
			if bone_name in ob.vertex_groups: continue
			
			bone_nameL = bone_name.lower()
			if self.skip_nub and bone_nameL.endswith('nub'): continue
			if ignores and bone_nameL in ignores: continue
